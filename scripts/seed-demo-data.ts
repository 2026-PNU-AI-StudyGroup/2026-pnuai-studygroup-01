...
const ids = {
  ...
  reportDefinitions: Array.from({ length: archivedProjectCount + 3 }, (_, index) => `f6000000-0000-4000-8000-${String(index + 1).padStart(12, "0")}`),
...
async function readFeedbackBoardSnapshot(tx: Prisma.TransactionClient) {
  const [snapshot] = await tx.$queryRaw<FeedbackBoardSnapshot[]>(Prisma.sql`
    WITH feedback_rows AS (
      SELECT 'feedback_post' AS "tableName", post."id"::text AS "rowId", row_to_json(post)::text AS payload
      FROM "feedback_post" AS post
      UNION ALL
      SELECT 'feedback_comment' AS "tableName", comment."id"::text AS "rowId", row_to_json(comment)::text AS payload
      FROM "feedback_comment" AS comment
      UNION ALL
      SELECT 'feedback_status_change' AS "tableName", change."id"::text AS "rowId", row_to_json(change)::text AS payload
      FROM "feedback_status_change" AS change
    )
    SELECT md5(COALESCE(string_agg(
      "tableName" || ':' || "rowId" || ':' || payload,
      E'\n' ORDER BY "tableName", "rowId"
    ), '')) AS fingerprint
    FROM feedback_rows
  `);
  if (!snapshot) throw new Error("피드백 게시판 보존 상태를 확인할 수 없습니다.");
...
async function seed() {
  ...
  const seedResult = await prisma.$transaction(async (tx) => {
    // The deployment seed resets only fixed demo fixtures. A changed feedback
    // snapshot rolls the whole transaction back instead of silently replacing
    // feedback-board content.
    const feedbackBoardBefore = await readFeedbackBoardSnapshot(tx);
    ...
    function defaultProgramSchedule(startsAt: Date, endsAt: Date) {
    ...
    await tx.announcement.createMany({ data: programAnnouncementRows });
    ...
    const activeReportDefinitions = [
      { id: ids.reportDefinitions[0], programId: activePrograms[0].id, title: "착수 보고서", dueAt: new Date("2026-04-10T23:59:59+09:00"), position: 0 },
      { id: ids.reportDefinitions[1], programId: activePrograms[0].id, title: "중간 보고서", dueAt: new Date("2026-06-12T23:59:59+09:00"), position: 1 },
      { id: ids.reportDefinitions[2], programId: activePrograms[0].id, title: "결과 보고서", dueAt: new Date("2026-08-14T23:59:59+09:00"), position: 2 },
    ];
    ...
    const [integrity] = await tx.$queryRaw<Array<{
      ...
      SELECT
        (SELECT count(*)::int FROM "project_program" AS program
          WHERE program."recruitmentStartsAt" < program."startsAt"
    ...
    if (reportsByTitle.get("착수 보고서")?.versions[0]?.decision?.decision !== "APPROVED"
      || reportsByTitle.get("중간 보고서")?.versions[0]?.decision?.decision !== "APPROVED"
      || reportsByTitle.get("결과 보고서")?.versions[0]?.decision !== null) {
    ...
    if (feedbackBoardBefore !== await readFeedbackBoardSnapshot(tx)) {
      throw new Error("데모 시드는 피드백 게시판 데이터를 변경할 수 없습니다.");
    }
    return {
      ...
      announcements: ids.announcements.length + programAnnouncementRows.length,
      studentDemoProject: {

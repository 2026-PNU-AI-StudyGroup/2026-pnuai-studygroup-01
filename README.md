...
확정된 팀은 할 일, 대화, 보고서와 결과물을 한 작업 공간에서 관리합니다. 종료된 프로젝트는 승인된 결과물과 함께 연도별 아카이브에 보존되어 다음 프로젝트의 참고 자료가 됩니다.
...
| 영역 | 기능 |
| --- | --- |
| 인증과 사용자 | Google OAuth 기반 `@pusan.ac.kr` 인증, 신규 학생 온보딩, 학생/교수/관리자/자문위원 역할, 자문위원 초대 토큰 로그인, 계정 비활성화와 세션 종료 |
| 프로그램 운영 | 시작일과 종료일을 정해 캡스톤, 대회, 교육 프로그램 개설, 공개와 마감 |
...
| 프로젝트 공간 | 할 일과 담당자, 팀과 지도교수 대화, 프로젝트 진행 현황 |
| 보고서와 결과물 | 보고서 요구사항과 기한, 버전 제출, 교수 승인과 수정 요청, SHA-256 검증 업로드, 공개 결과물 등록 |
| 종료와 아카이브 | 프로그램 종료일에 따른 완료와 취소 판정, 연도별 완료 프로젝트와 공개 결과물 보존 |
| 알림과 감사 | 앱 내 알림과 이메일 알림, 권한 변경과 팀 확정, 보고서 승인 등 중요 운영 행위 기록 |
| 다국어 | 화면 문구 한영 카탈로그, 계정별 언어 설정, 원문 확인. 사용자 콘텐츠 자동 번역(PostgreSQL 번역 큐 + 로컬 Ollama)은 기본 꺼짐 |
...
웹 화면, Server Action과 Route Handler는 하나의 Next.js 애플리케이션에서 운영합니다. PostgreSQL은 업무 데이터와 번역, 이메일 대기열을 저장하고, MinIO는 업로드 파일을 보관합니다. 번역 워커만 로컬 Ollama를 호출하며 외부 AI API는 사용하지 않습니다. **사용자 콘텐츠 자동 번역은 기본으로 꺼져 있어** 다이어그램에서도 점선으로 표시했습니다. 이메일 워커는 Gmail SMTP로 발송합니다.
...
프로그램 아래에 여러 주제가 달리고, 주제 하나에는 확정 팀이 최대 하나 붙습니다. 팀은 팀원 소속과 보고서, 업로드 파일을 가지며 투표는 주제와 투표자에 각각 연결됩니다. 전체 65개 모델 중 프로젝트의 뼈대가 되는 8개만 표시했습니다.
...
- 초대 토큰 링크로 접속(부산대학교 계정 없이 참여)
- 배정된 팀의 프로젝트와 제출물 열람
- 채점표 항목별 채점과 피드백 작성
- 관리자와 동일한 기본 5표 투표 한도
...
- [다이어그램 원본](docs/diagrams) - 편집 가능한 HTML 과 내보낸 SVG - PNG

...
기획부터 매뉴얼 작성까지 **Claude Code**(Anthropic) 하나로 진행했습니다. 여러 도구를 옮겨 다니는 대신 저장소를 직접 읽고 고치는 도구 하나에 붙었습니다. 코드와 문서가 한자리에서 같이 바뀌어야 서로 어긋나지 않는다고 봤기 때문입니다.

| 단계 | 활용 내용 |
| --- | --- |
| 설계 | `app`(화면)과 `modules`(업무 규칙)의 경계, 계층 간 의존 방향을 정하고 그 규칙을 테스트로 굳혔습니다 |
| 구현 | 프로그램 개설, 팀 구성과 메일 초대, 보고서 승인, 인기상 투표, 다국어, 다크 모드까지 기능 단위로 만들었습니다 |
| 결함 점검 | 학생·교수·관리자·자문위원 입장으로 화면을 차례로 눌러 보며 결함을 모았습니다 |
| 회귀 시험 | 고친 자리마다 테스트를 남겨 같은 문제가 다시 들어오지 못하게 했습니다 |
| 문서 | 이 보고서와 설계 다이어그램, 운영 매뉴얼을 코드와 같이 고쳤습니다 |

여기까지 쌓인 결과는 아래와 같습니다.

| 항목 | 값 |
| --- | --- |
| 커밋 | 562개 |
| 병합한 풀 리퀘스트 | 90건 |
| 소스 파일 | 767개 (그중 테스트 260개) |
| 자동 검증 규칙 | 아키텍처 테스트 5종 |
| 영문 문구 카탈로그 | 2,369개 항목 |
| 설계 다이어그램 | 7종 |

**가장 크게 남은 것은 사람이 일일이 지키기 어려운 규칙을 기계가 지키게 만든 일입니다.** 한국어 문구를 새로 넣으면 영문 카탈로그에 짝이 없다는 이유로 테스트가 깨지고, 화면 코드가 업무 규칙을 거꾸로 참조하면 의존 방향 검사가 막습니다. 규칙을 문서로만 적어 두었다면 몇 주 만에 흐트러졌을 것입니다.

다만 도구가 내놓은 것을 그대로 받지는 않았습니다. 지적받은 것 중에는 실제 문제가 아닌 것도 섞여 있어서, 데이터베이스에 같은 상황을 만들어 재현되는 것만 고쳤습니다. 앞서 적은 투표 동시성 문제가 그렇게 잡은 경우입니다. 무엇을 남기고 무엇을 버릴지 정하는 일은 사람 몫으로 두었습니다.

서비스 안에도 언어 모델이 한 자리 들어가 있습니다. 화면에 박힌 문구는 위의 영문 카탈로그로 바꾸고, 사용자가 직접 쓴 글은 로컬에서 돌리는 `qwen3.5:2b`가 옮기도록 해 두었습니다. 외부로 글을 내보내지 않으려고 로컬 모델을 골랐고, 기본은 꺼 둔 상태입니다.
...
여섯 명이 각자 다른 구간을 맡았습니다. 개발만 나눈 것이 아니라 역할별 사용자 테스트와 매뉴얼 작성까지 사람을 붙였는데, 만든 사람은 길을 다 알고 있어서 막히는 자리를 스스로 찾지 못한다는 것을 중간에 알게 됐기 때문입니다.

| 이름 | 담당 |
| --- | --- |
| 이태경 | 총괄. 목록 탐색 개선, 필터 즉시 적용, 정렬과 페이지네이션, 알림과 이메일, 운영 서버 배포와 장애 대응 |
| 양준영 | 시스템 설계와 모듈 경계, 인증과 권한, 주제 등록과 지원, 팀 확정, 학생 팀과 팀원 모집, 프로그램 운영, 보고서 제출과 승인, 사용자 콘텐츠 번역 |
| 서연우 | 작성 중 내용 임시 저장(프로그램, 주제), 팀원 모집 공고 화면, 파일 업로드와 저장소 |
| 문보미 | 관리자 화면 사용성 점검과 진입 동선 보완, 온라인 투표 |

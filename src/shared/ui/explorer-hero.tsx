    ...
    <section aria-labelledby="explorer-hero-title" className="border-b border-[var(--line)] pb-7">
      <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          {mark ? <span aria-hidden="true" className="grid size-14 shrink-0 place-items-center rounded-full border border-[var(--primary)]/20 bg-white text-lg font-bold text-[var(--primary)] shadow-[0_8px_24px_rgba(47,91,234,.08)]">
            {mark}
          </span> : null}
        ...
        {action ? <div className={`flex shrink-0 ${mark ? "pl-[4.5rem]" : ""} sm:pl-0`}>{action}</div> : null}
      </div>
    </section>
  );
}

...
vi.mock("@/app/projects/[projectId]/_actions/project-guidance-request-actions", () => ({
...
describe("프로젝트 지도 요청 폼", () => {
  ...
  it("회의와 검토를 전환하며 공통 요청 입력 제약을 적용한다", () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-08-03T05:00:00Z"));

    const { container } = render(
      <ProjectGuidanceRequestForm teamId={teamId} />,
    );
  ...
  it("요청 액션에 숨김 필드와 입력을 전달하고 성공 후 폼을 초기화한다", async () => {
    ...
    expect(formData.get("preferredAt")).toBeNull();

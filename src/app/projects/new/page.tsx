...
export const metadata: Metadata = { title: "학생 프로젝트 만들기" };
...
export default async function NewStudentProjectPage() {
  ...
  const [programs, professors] = await Promise.all([new ProjectProgramService(programRepository).listOpen(), approvalService.listProfessors()]);
  return <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/projects/new"><main className="content-shell page-enter space-y-8">
    <PageHeader eyebrow="학생 제안" title="새 프로젝트 만들기" description="학생이 제안한 프로젝트는 지정 교수 또는 관리자 승인을 받은 뒤 공개됩니다." actions={<Link href="/project-approvals" className="button-secondary">내 승인 요청</Link>} />
    {programs.length ? <TopicForm action={createTopicAction} programs={programs} successHref="/project-approvals" studentApproval={{ professors }} /> : <EmptyState title="지금 제안할 수 있는 프로그램이 없습니다" description="공개 프로그램이 시작되면 프로젝트를 제안할 수 있습니다." />}
  </main></AppShell>;
}

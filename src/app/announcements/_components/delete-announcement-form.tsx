...
export function DeleteAnnouncementForm({
  ...
  return (
    <form action={action} className="flex flex-col items-end gap-2">
      ...
      <ConfirmSubmitButton
        className={iconOnly ? `${iconControlClassName} text-[var(--danger)] hover:text-[var(--danger)]` : "button-danger"}
        aria-label={iconOnly ? "공지 삭제" : undefined}
        title={iconOnly ? "공지 삭제" : undefined}
        confirmMessage="이 공지사항을 삭제하시겠습니까? 삭제한 공지는 복구할 수 없습니다."
        disabled={pending}
      >
        {iconOnly ? <TrashIcon className="size-5" /> : <UiText>{pending ? "삭제 중" : "삭제"}</UiText>}
      </ConfirmSubmitButton>
      {state.message ? (

...
export default async function ProfessorTopicsPage() {
  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/professor/topics">
      <ProfessorWorkspace currentPath="/professor/topics" title="주제 설계" description="학생이 도전할 프로젝트의 목표와 지원 조건, 일정을 설계합니다." actions={<>{programs.length ? <Link href="/professor/topics/new" className="button-primary"><UiText>{"새 주제 만들기"}</UiText></Link> : null}<Link href="/project-approvals" className="button-secondary"><UiText>{"학생 제안 검토"}</UiText></Link><Link href="/professor/applications" className="button-secondary"><UiText>{"지원 검토"}</UiText></Link></>}>
        {programs.length === 0 ? <p role="status" className="border-l-2 border-[var(--warning)] bg-[var(--warning-subtle)] p-4 text-sm text-[var(--warning-ink)]"><UiText>{"공개된 프로그램이 생기면 새 주제를 만들 수 있습니다."}</UiText></p> : null}
        <section aria-labelledby="topic-list-title">
          <div className="flex items-end justify-between pb-4"><h2 id="topic-list-title" className="text-lg font-bold"><UiText>{actor.role === "ADMIN" ? "전체 주제" : "내 주제"}</UiText></h2><span className="muted text-sm">{topics.length}<UiText>{"개"}</UiText></span></div>

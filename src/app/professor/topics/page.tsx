...
export default async function ProfessorTopicsPage() {
  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/professor/topics">
      <ProfessorWorkspace currentPath="/professor/topics" title="주제 준비" description="프로그램에 맞는 주제를 작성하고, 학생에게 공개할 지원 조건과 운영 기간을 관리합니다." actions={<>{programs.length ? <Link href="/professor/topics/new" className="button-primary">새 주제 등록</Link> : null}<Link href="/professor/applications" className="button-secondary">지원 검토로</Link></>}>

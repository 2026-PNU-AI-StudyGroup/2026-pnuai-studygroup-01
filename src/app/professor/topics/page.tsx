...
export default async function ProfessorTopicsPage() {
  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/professor/topics">
      <ProfessorWorkspace currentPath="/professor/topics" title="주제 설계" description="학생이 도전할 프로젝트의 목표와 지원 조건, 일정을 설계합니다." actions={<>{programs.length ? <Link href="/professor/topics/new" className="button-primary">새 주제 만들기</Link> : null}<Link href="/project-approvals" className="button-secondary">학생 제안 검토</Link><Link href="/professor/applications" className="button-secondary">지원 검토</Link></>}>
        ...
        <section aria-labelledby="topic-list-title">
          ...
          {topics.length === 0 ? <div className="mt-6"><EmptyState title={actor.role === "ADMIN" ? "아직 만들어진 주제가 없습니다" : "아직 만든 주제가 없습니다"} description={programs.length ? "첫 프로젝트 주제를 설계해 학생에게 공개해 보세요." : "공개된 프로그램이 생기면 새 주제를 만들 수 있습니다."} action={programs.length ? <Link className="button-primary" href="/professor/topics/new">새 주제 만들기</Link> : undefined} /></div> : (
            <ul className="divide-y divide-[var(--line)] border-y border-[var(--line)] bg-white">
              {topics.map((topic) => <li key={topic.id} className="record-row grid gap-5 py-6 lg:grid-cols-[minmax(0,1fr)_11rem_13rem_auto] lg:items-center"><div><div className="flex flex-wrap items-center gap-3"><h3 className="text-lg font-semibold tracking-[-0.02em]">{topic.title}</h3><StatusBadge tone={statusPresentation[topic.status][1]}>{statusPresentation[topic.status][0]}</StatusBadge></div><p className="muted mt-1 text-sm">{topic.programName} · {topic.programCategory}{actor.role === "ADMIN" ? ` · ${topic.authorName} 교수` : ""}</p></div><dl className="grid grid-cols-[5rem_1fr] gap-1 text-sm lg:block"><dt className="muted lg:text-xs">모집 정원</dt><dd className="lg:mt-1">{topic.capacity}명</dd></dl><dl className="grid grid-cols-[5rem_1fr] gap-1 text-sm lg:block"><dt className="muted lg:text-xs">모집 마감</dt><dd className="lg:mt-1">{koreanDateTime.format(topic.recruitmentEndsAt)}</dd></dl><div className="flex flex-wrap gap-2 lg:justify-end"><Link href={`/professor/topics/${topic.id}`} className="button-secondary">상세</Link><TopicStatusButton topicId={topic.id} status={topic.status} programStatus={topic.programStatus} /></div></li>)}
            </ul>
          )}
        </section>
      </ProfessorWorkspace>

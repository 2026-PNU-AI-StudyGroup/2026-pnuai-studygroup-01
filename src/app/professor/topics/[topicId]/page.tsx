...
export default async function ManagedTopicPage({ params }: { params: Promise<{ topicId: string }> }) {
  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/professor/topics">
      <ProfessorWorkspace currentPath={`/professor/topics/${topic.id}`} role={actor.role} eyebrow={topic.programName} title={topic.title} actions={<><Link href={`/professor/topics/${topic.id}/assistants`} className="button-secondary"><UiText>{"조교 관리"}</UiText></Link>{topic.status === "PUBLISHED" ? <Link href={`/professor/topics/${topic.id}/edit`} className="button-secondary"><UiText>{"내용 편집"}</UiText></Link> : null}{topic.status === "PUBLISHED" ? <Link href={`/professor/topics/${topic.id}/schedule`} className="button-secondary"><UiText>{"일정 편집"}</UiText></Link> : null}<TopicStatusButton topicId={topic.id} status={topic.status} pendingApplicationCount={topic.pendingApplicationCount} openRecruitmentPostCount={topic.openRecruitmentPostCount} recruitmentEnabled={topic.recruitmentEnabled} canCloseRecruitment={actor.role === "ADMIN" || topic.managerId === actor.id} /></>}>

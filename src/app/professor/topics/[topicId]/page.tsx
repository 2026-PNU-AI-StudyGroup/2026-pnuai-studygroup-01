...
export default async function ManagedTopicPage({ params }: { params: Promise<{ topicId: string }> }) {
  ...
  return <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/professor/topics"><ProfessorWorkspace currentPath={`/professor/topics/${topic.id}`} eyebrow={topic.programName} title={topic.title} description="지원자에게 공개되는 내용과 지원서, 운영 일정을 한 화면에서 최종 점검합니다." actions={<><Link href="/professor/topics" className="button-secondary"><UiText>{"주제 목록"}</UiText></Link>{topic.status !== "CLOSED" ? <Link href={`/professor/topics/${topic.id}/schedule`} className="button-secondary"><UiText>{"일정 편집"}</UiText></Link> : null}<TopicStatusButton topicId={topic.id} status={topic.status} programStatus={topic.programStatus} /></>}>

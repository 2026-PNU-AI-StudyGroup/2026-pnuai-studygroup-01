...
export async function cancelProjectAssistantInvitationAction(formData: FormData) {
  const actor = await getCurrentActor();
  if (!actor) redirect("/sign-in");
  const parsed = z.object({
    invitationId: z.string().uuid(),
  ...
  try {
    ...
    if (error instanceof ProjectAssistantOperationError) return;
    throw error;
  }
  refresh(parsed.data.topicId);
}

export async function removeProjectAssistantAction(formData: FormData) {
  const actor = await getCurrentActor();
  if (!actor) redirect("/sign-in");
  const parsed = z.object({
    topicId: z.string().uuid(),

  ...
  return (
    <AppShell role={actor.role} userName="부산대학교" currentPath="/dashboard">
      <main className="content-shell space-y-10">
        <PageHeader eyebrow="Project workspace" title={title} description="팀의 현재 진행 상태, 마일스톤, 보고서 제출과 다음 작업을 확인하세요." actions={<Link href={actor.role === "STUDENT" ? "/topics" : "/professor/topics"} className="button-primary">{actor.role === "STUDENT" ? "새 주제 탐색" : "주제 관리"}</Link>} />
        {teams.length === 0 ? (

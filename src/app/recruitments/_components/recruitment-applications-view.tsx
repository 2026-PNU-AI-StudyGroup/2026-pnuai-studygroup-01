  ...
  return (
    <div className="space-y-8">
      <StudentTeamPageIntro
        ...
        meta={<><span>{post.topicTitle}</span><span aria-hidden="true">·</span><strong className="text-[var(--ink)]">{post.teamName}</strong></>}
        action={
      ...
      <div className="overflow-hidden rounded-[var(--radius-panel)] border border-[var(--line)] bg-white">
        <section aria-labelledby="recruitment-content-title" className="grid gap-6 border-b border-[var(--line)] px-7 py-7 lg:grid-cols-[12rem_minmax(0,1fr)] lg:px-9">
          <div>
            <div className="flex items-center gap-3">
              <h2 id="recruitment-content-title" className="text-base font-black text-[var(--ink)]">모집 내용</h2>
              <StatusBadge tone={post.status === "OPEN" ? "success" : undefined}>{post.status === "OPEN" ? "모집 중" : "모집 종료"}</StatusBadge>
            </div>
            <p className="mt-2 text-sm text-[var(--muted)]">지원자 {post.applications.length}명 · 대기 {pendingCount}명</p>
          </div>
        ...
        <section aria-labelledby="applicant-list-title">
          <div className="border-b border-[var(--line)] bg-[var(--surface-subtle)] px-7 py-4 lg:px-9">
            <h2 id="applicant-list-title" className="text-sm font-black text-[var(--ink)]">지원자</h2>
          </div>
                ...
                <li key={application.id} className="border-b border-[var(--line)] px-7 py-7 last:border-b-0 lg:px-9">
                  <article className="grid gap-6 lg:grid-cols-[11rem_minmax(0,1fr)_auto]">
                    <div>
                      ...
                      <p className="mt-2 text-xs text-[var(--muted)]">{koreanDate.format(application.createdAt)} 지원</p>
                    </div>
                    <div className="min-w-0">
                      <dl className="grid gap-5 text-sm sm:grid-cols-3">
                        <div><dt className="text-xs font-semibold text-[var(--muted)]">보유 기술</dt><dd className="mt-1 font-bold">{application.skills.join(", ")}</dd></div>
                        <div><dt className="text-xs font-semibold text-[var(--muted)]">희망 역할</dt><dd className="mt-1 font-bold">{application.desiredRole}</dd></div>
                        <div><dt className="text-xs font-semibold text-[var(--muted)]">활동 가능 시간</dt><dd className="mt-1 font-bold">{application.availability}</dd></div>
                      </dl>
                      <div className="mt-5 border-t border-[var(--line)] pt-4">
                        <p className="text-xs font-semibold text-[var(--muted)]">지원 메시지</p>

...
export default async function RecruitmentsPage({ searchParams }: { searchParams: Promise<{ page?: SearchParamValue; historyPage?: SearchParamValue }> }) {
  ...
  return <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/recruitments">
    <main className="content-shell space-y-10">
      ...
      {data.posts.length === 0 ? <EmptyState title="열린 모집 글이 없습니다" description="구성 중인 팀에서 모집 글을 등록하면 이곳에 표시됩니다." /> : <>
        ...
        <nav aria-label="모집 글 페이지" className="flex flex-wrap items-center justify-between gap-3"><span className="muted text-sm">{data.page} / {data.totalPages} 페이지 · 총 {data.total}개</span><div className="flex gap-2">{data.page > 1 ? <Link className="button-quiet" href={pageHref(data.page - 1, data.historyPage)}>이전</Link> : null}{data.page < data.totalPages ? <Link className="button-quiet" href={pageHref(data.page + 1, data.historyPage)}>다음</Link> : null}</div></nav>
      </>}

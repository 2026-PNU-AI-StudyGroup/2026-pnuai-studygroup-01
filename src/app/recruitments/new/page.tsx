    ...
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/recruitments/new">
      <main className="content-shell">
        <RecruitmentSectionLayout currentPath="/recruitments/new"><div className="space-y-10">
          <PageHeader eyebrow="팀원 찾기" title="새 모집 글" description="필요한 기술과 역할, 함께 활동할 수 있는 시간을 명확히 적어 팀에 맞는 지원자를 찾습니다." actions={<Link className="button-quiet" href="/recruitments">모집 목록으로</Link>} />
          {teams.length ? <RecruitmentPostForm teams={teams} successHref="/recruitments" /> : <EmptyState title="모집 글을 등록할 팀이 없습니다" description="모집 기간 중인 구성 단계 팀에 먼저 참여해야 모집 글을 작성할 수 있습니다." action={<Link className="button-secondary" href="/recruitments">모집 목록으로</Link>} />}

...
export function TopicApprovalDecisionForm({ requestId }: { requestId: string }) {
  ...
  return <form action={action} className="grid gap-3">
    ...
    <label className="grid gap-2 text-sm font-semibold"><UiText>{"검토 의견"}</UiText><UiTextarea className="field" name="reviewComment" maxLength={1000} rows={2} placeholder="승인 또는 반려 사유를 남길 수 있습니다." /></label>

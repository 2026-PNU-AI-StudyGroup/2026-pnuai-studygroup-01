...
export function TopicApprovalDecisionForm({ requestId, studentTeamVersion }: { requestId: string; studentTeamVersion?: number | null }) {
  ...
  return <form action={action} className="grid gap-3">
    ...
    {studentTeamVersion ? <label className="flex items-start gap-2 rounded-[var(--radius-control)] bg-[var(--surface)] p-3 text-sm leading-6 text-[var(--ink)]"><input className="mt-1 size-4" type="checkbox" name="teamCompositionConfirmed" /><span><UiText>{"표시된 현재 팀 구성을 확인했습니다. 승인하면 이 인원으로 실행 팀이 즉시 만들어집니다."}</UiText></span></label> : null}

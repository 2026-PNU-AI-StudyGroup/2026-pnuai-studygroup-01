  ...
  return (
    <section aria-labelledby="approval-ledger-title">
      <div className="mb-4 flex items-end justify-between gap-4">
        <div>
          <h2 id="approval-ledger-title" className="text-xl font-black tracking-[-0.03em]">
            {student ? "보낸 요청" : "승인 대기열"}
          </h2>
          <p className="mt-1 text-sm text-[var(--muted)]">
            {student ? "요청 경로와 검토 결과를 확인합니다." : "제안 내용과 요청 경로를 확인한 뒤 처리합니다."}
          </p>
        </div>
        <span className="shrink-0 text-sm font-bold text-[var(--muted)]">{requests.length}건</span>
      </div>

      <div aria-hidden="true" className={`hidden border-y border-[var(--line)] px-2 py-3 text-xs font-bold text-[var(--muted)] xl:grid xl:gap-5 ${approvalLedgerColumns}`}>
        <span>프로젝트</span>
        <span>상태</span>
        <span>제안자</span>
        <span>요청 경로</span>
        <span>{student ? "검토 의견" : "검토"}</span>
      </div>
      <ul aria-label="프로젝트 승인 요청 목록" className="divide-y divide-[var(--line)] border-y border-[var(--line)] bg-white xl:border-t-0">
          ...
          return (
            <li key={request.id} className={`record-row grid gap-5 px-2 py-6 xl:items-start xl:gap-5 ${approvalLedgerColumns}`}>
              <div className="min-w-0">
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden">프로젝트</p>
                <h3 className="text-lg font-semibold tracking-[-0.02em] text-[var(--ink)]">{request.topicTitle}</h3>
              </div>
              <div>
                <p className="mb-1.5 text-xs font-bold text-[var(--muted)] xl:hidden">상태</p>
                <StatusBadge tone={status[request.status][1]}>{status[request.status][0]}</StatusBadge>
              </div>
              <div>
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden">제안자</p>
                <p className="text-sm font-semibold">{request.requesterName}</p>
              </div>
              <div>
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden">요청 경로</p>
                <p className="text-sm font-semibold">{route}</p>
              </div>
              <div className="min-w-0">
                <p className="mb-2 text-xs font-bold text-[var(--muted)] xl:hidden">{student ? "검토 의견" : "검토"}</p>
                {showDecision ? (

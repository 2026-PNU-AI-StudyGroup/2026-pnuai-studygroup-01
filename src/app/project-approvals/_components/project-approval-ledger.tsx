          ...
          return (
            <li key={request.id} className={`record-row grid gap-5 px-2 py-6 xl:items-start xl:gap-5 ${approvalLedgerColumns}`}>
              <div className="min-w-0">
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden"><UiText>{"프로젝트"}</UiText></p>
                <h3 className="text-lg font-semibold tracking-[-0.02em] text-[var(--ink)]"><UiText>{request.topicTitle}</UiText></h3>
              </div>
              <div>
                <p className="mb-1.5 text-xs font-bold text-[var(--muted)] xl:hidden"><UiText>{"상태"}</UiText></p>
                <StatusBadge tone={status[request.status][1]}>{status[request.status][0]}</StatusBadge>
              </div>
              <div>
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden"><UiText>{"제안자"}</UiText></p>
                <p className="text-sm font-semibold">{request.requesterName}</p>
              </div>
              <div>
                <p className="mb-1 text-xs font-bold text-[var(--muted)] xl:hidden"><UiText>{"요청 경로"}</UiText></p>
                <p className="text-sm font-semibold">{route}</p>
              </div>
              <div className="min-w-0">
                <p className="mb-2 text-xs font-bold text-[var(--muted)] xl:hidden"><UiText>{student ? "검토 의견" : "검토"}</UiText></p>
                {showDecision ? (

...
describe("보고서 화면 표현 상태", () => {
  it("승인된 이른 보고서보다 수정 요청이 있는 보고서를 지금 할 일로 고른다", () => {
    const approved = report({ id: "start", type: "START", dueAt: "2026-08-31T00:00:00Z", decision: "APPROVED" });
    const revision = report({ id: "midterm", type: "MIDTERM", dueAt: "2026-10-15T00:00:00Z", decision: "REVISION_REQUESTED" });
  ...
  it("승인된 보고서의 마감보다 이후라도 미제출 보고서를 우선한다", () => {
    const approved = report({ id: "start", type: "START", dueAt: "2026-08-31T00:00:00Z", decision: "APPROVED" });
    const unsubmitted = report({ id: "final", type: "FINAL", dueAt: "2026-12-10T00:00:00Z", submitted: false });
  ...
  it("최신 버전에 결정이 없으면 과거 결정과 무관하게 검토 대기로 분류한다", () => {
    const pending = report({ id: "midterm", type: "MIDTERM", dueAt: "2026-10-15T00:00:00Z", decision: null });
  ...
  it("마감 시각까지는 제출 가능하고 그 이후에는 차단 상태를 고른다", () => {
    const unsubmitted = report({ id: "final", type: "FINAL", dueAt: "2026-08-04T00:00:00Z", submitted: false });

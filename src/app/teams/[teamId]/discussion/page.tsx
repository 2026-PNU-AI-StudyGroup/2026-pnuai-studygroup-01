...
export default async function TeamDiscussionPage({ params, searchParams }: { params: Promise<{ teamId: string }>; searchParams: Promise<{ page?: SearchParamValue }> }) {
  ...
  return <section aria-labelledby="discussion-title" className="space-y-8"><header className="border-b border-[var(--line)] pb-7"><p className="eyebrow">함께 나누기</p><h1 id="discussion-title" className="mt-2 text-3xl font-black tracking-[-0.04em]">팀 대화</h1><p className="muted mt-2">아이디어와 피드백을 한곳에 남기고, 필요한 글만 번역해 보세요.</p></header>{workspace.status !== "CLOSED" ? <DiscussionPostForm teamId={workspace.id} /> : null}{workspace.discussionPosts.length === 0 ? <EmptyState title="아직 나눈 대화가 없습니다" description={emptyDescription} /> : <ol className="border-t border-[var(--primary)]">{workspace.discussionPosts.map((post) => <li key={post.id} className="grid gap-3 border-b border-[var(--line)] py-6 sm:grid-cols-[8rem_minmax(0,1fr)]"><div><strong className="text-sm">{post.authorName}</strong><time className="muted mt-1 block text-xs" dateTime={post.createdAt.toISOString()}>{koreanDate.format(post.createdAt)}</time></div><TranslatedText text={post.content} className="leading-7" /></li>)}</ol>}{workspace.discussionTotalPages > 1 ? <nav aria-label="팀 대화 페이지" className="flex flex-wrap items-center justify-between gap-3"><span className="muted text-sm">{workspace.discussionPage} / {workspace.discussionTotalPages} 페이지 · 총 {workspace.discussionTotal}개</span><div className="flex gap-2">{workspace.discussionPage > 1 ? <Link className="button-quiet" href={`/teams/${teamId}/discussion?page=${workspace.discussionPage - 1}`}>최근 대화</Link> : null}{workspace.discussionPage < workspace.discussionTotalPages ? <Link className="button-quiet" href={`/teams/${teamId}/discussion?page=${workspace.discussionPage + 1}`}>이전 대화</Link> : null}</div></nav> : null}</section>;
}

  ...
  return (
    <AppShell role={actor.role} userName="부산대학교" currentPath="/dashboard">
      <main className="content-shell space-y-12">
        <PageHeader eyebrow="Team workspace" title={workspace.name} description={`${workspace.topicTitle} · 지도교수 ${workspace.professorName}`} actions={<div className="flex items-center gap-3"><StatusBadge tone={workspace.status === "CONFIRMED" ? "success" : "neutral"}>{workspace.status === "FORMING" ? "구성 중" : workspace.status === "CONFIRMED" ? "확정 팀" : "종료 팀"}</StatusBadge>{workspace.status === "FORMING" && actor.role !== "STUDENT" ? <form action={confirmTeamAction}><input type="hidden" name="teamId" value={workspace.id} /><button className="button-primary">팀 확정</button></form> : null}{workspace.status === "CONFIRMED" && workspace.canClose && actor.role !== "STUDENT" ? <CloseTeamForm teamId={workspace.id} /> : null}<div className="w-48"><ProgressBar value={progress} /></div></div>} />

  ...
  return (
    <div className="space-y-10">
      ...
      <section aria-labelledby="next-action-title" className="grid gap-3 border-b border-[var(--line)] pb-7 sm:grid-cols-[8rem_minmax(0,1fr)_auto] sm:items-center">
        <p className="text-xs font-bold text-[var(--primary)]"><UiText>{"다음 일정"}</UiText></p>
        <h2 id="next-action-title" className="text-lg font-bold"><UiText>{nextMilestone?.title ?? "등록된 마일스톤이 없습니다"}</UiText></h2>
        <time className="text-sm font-semibold" dateTime={nextMilestone?.dueAt.toISOString()}>
      ...
      {workspace.access.canSupervise && workspace.status !== "CLOSED" ? (
        <section aria-labelledby="professor-actions-title" className="grid gap-5 border-y border-[var(--line)] bg-[var(--primary-subtle)] px-5 py-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
          <div>
            ...
            <h2 id="professor-actions-title" className="mt-1 text-lg font-bold"><UiText>{"팀 피드백과 보고서 검토"}</UiText></h2>
      ...
      <section aria-labelledby="schedule-title">
        <div className="border-b border-[var(--line)] pb-3"><p className="eyebrow"><UiText>{"일정"}</UiText></p><h2 id="schedule-title" className="mt-1 text-xl font-bold"><UiText>{"프로젝트 기간"}</UiText></h2></div>
        <ol className="grid border-b border-[var(--line)] sm:grid-cols-3">
          {schedule.map(([label, start, end]) => (
            <li key={label} className="relative border-b border-[var(--line)] py-5 last:border-b-0 sm:border-b-0 sm:border-r sm:px-5 sm:first:pl-0 sm:last:border-r-0">
              ...
              <h3 className="mt-3 font-bold"><UiText>{label}</UiText> {" "}<UiText>{"기간"}</UiText></h3>
      ...
      <section aria-labelledby="workspace-destinations">
        <div className="border-b border-[var(--line)] pb-3"><p className="eyebrow"><UiText>{"작업 영역"}</UiText></p><h2 id="workspace-destinations" className="mt-1 text-xl font-bold"><UiText>{"프로젝트 작업"}</UiText></h2></div>
          ...
          {destinations.map(([title, description, href, meta]) => (
            <li key={href}>
              <Link href={href} className="group grid min-h-20 gap-2 py-4 sm:grid-cols-[9rem_minmax(0,1fr)_auto] sm:items-center">
                <h3 className="font-bold group-hover:text-[var(--primary-hover)]"><UiText>{title}</UiText></h3>
      ...
      <section aria-labelledby="members-title">
        <div className="border-b border-[var(--line)] pb-3"><p className="eyebrow"><UiText>{"구성원"}</UiText></p><h2 id="members-title" className="mt-1 text-xl font-bold"><UiText>{"팀원"}</UiText>{" "}{workspace.members.length}<UiText>{"명"}</UiText></h2></div>

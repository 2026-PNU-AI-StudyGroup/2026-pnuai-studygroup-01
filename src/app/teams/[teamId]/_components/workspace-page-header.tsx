  ...
  return (
    <header className="flex flex-wrap items-end justify-between gap-5 border-b border-[var(--line)] pb-6">
      <div className="max-w-2xl">
        ...
        <h1 id={titleId} className="mt-2 text-[clamp(1.75rem,4vw,2.25rem)] font-black leading-tight tracking-[-0.045em]">{title}</h1>
        <p className="muted mt-2 max-w-xl text-sm leading-6 sm:text-base">{description}</p>
      </div>
...
export function MobileFieldLabel({ children }: { children: ReactNode }) {
  return <span className="mb-1 block text-[0.6875rem] font-bold uppercase tracking-[0.06em] text-[var(--muted)] md:hidden">{children}</span>;
}

          ...
          <ol className="border-b border-[var(--line)]">
            {workspace.milestones.map((milestone) => (
              <li key={milestone.id} className="relative grid gap-4 border-b border-[var(--line)] px-2 py-5 last:border-b-0 xl:grid-cols-[7rem_minmax(0,1fr)_9rem_10rem_19rem] xl:items-center">
                ...
                <div className="xl:justify-self-end">{workspace.status !== "CLOSED" && workspace.access.canContribute ? <MilestoneStatusForm teamId={workspace.id} milestoneId={milestone.id} status={milestone.status} assigneeIds={milestone.assignees.map(({ id }) => id)} members={workspace.members} /> : null}</div>
              </li>
            ))}
          </ol>
        </div>

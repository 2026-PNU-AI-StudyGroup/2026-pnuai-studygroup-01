  ...
  return (
    <section aria-labelledby="milestones-title" className="space-y-8">
      <WorkspacePageHeader
        ...
        meta={<p className="text-sm"><strong className="text-[var(--primary)]">{workspace.completedMilestoneCount}</strong> / {workspace.milestoneCount} 완료</p>}
      />
      ...
      {workspace.milestones.length === 0 ? <EmptyState title="마일스톤이 없습니다" description={emptyDescription} /> : (
        <div>
          <div className="hidden grid-cols-[7rem_minmax(0,1fr)_9rem_10rem_19rem] border-b border-[var(--line-strong)] px-2 pb-3 text-xs font-bold text-[var(--muted)] xl:grid">
            <span>상태</span><span>마일스톤</span><span>담당자</span><span>완료 예정</span><span className="text-right">변경</span>
          </div>
          <ol className="border-b border-[var(--line)]">
            {workspace.milestones.map((milestone) => (
              <li key={milestone.id} className="relative grid gap-4 border-b border-[var(--line)] px-2 py-5 last:border-b-0 xl:grid-cols-[7rem_minmax(0,1fr)_9rem_10rem_19rem] xl:items-center">
                ...
                <div><MobileFieldLabel>상태</MobileFieldLabel><StatusBadge tone={milestoneStatus[milestone.status][1]}>{milestoneStatus[milestone.status][0]}</StatusBadge></div>
                <div><MobileFieldLabel>마일스톤</MobileFieldLabel><p className="font-semibold">{milestone.title}</p></div>
                <div><MobileFieldLabel>담당자</MobileFieldLabel><p className="text-sm font-semibold">{milestone.assignees.map(({ name }) => name).join(", ") || "미지정"}</p></div>
                <div><MobileFieldLabel>완료 예정</MobileFieldLabel><time className="text-sm font-semibold md:text-[var(--muted)]" dateTime={milestone.dueAt.toISOString()}>{koreanDate.format(milestone.dueAt)}</time></div>

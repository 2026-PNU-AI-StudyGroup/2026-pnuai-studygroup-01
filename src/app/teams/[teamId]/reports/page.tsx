  ...
  return (
    <article id={`report-${report.id}`} className="scroll-mt-6 rounded-[var(--radius-panel)] border border-[var(--line)] bg-[var(--surface)] p-5 shadow-sm sm:p-6">
      ...
      {report.versions.length ? (
        ...
        <p className="mt-5 rounded-2xl bg-[var(--surface-subtle)] p-4 text-sm text-[var(--muted)]"><UiText>{"아직 제출된 버전이 없습니다."}</UiText></p>
      )}
...
export default async function TeamReportsPage({ params }: { params: Promise<{ teamId: string }> }) {
  ...
  const noSubmittableReports = workspace.status === "CONFIRMED" && workspace.access.canContribute && requiredReports.length > 0 && requiredReports.every((report) => !isReportSubmissionOpen(report, now));
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {submittedCount ? <a href={`/api/teams/${workspace.id}/submissions`} className="button-secondary"><UiText>{"제출물 전체 다운로드"}</UiText></a> : null}
          ...
          {reportWorkspace.reports.map((report) => (
            <ReportCard
              ...
              teamId={workspace.id}
              report={report}
              canSubmit={workspace.status === "CONFIRMED" && workspace.access.canContribute && report.required && isReportSubmissionOpen(report, now)}
              canReview={canReview}

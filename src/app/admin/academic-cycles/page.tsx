  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/admin/academic-cycles">
      <AdminWorkspace currentPath="/admin/academic-cycles" title="운영 학기" description="모든 프로그램이 공유할 학년도와 학기 기준을 설정합니다." actions={<Link className="button-primary" href="/admin/academic-cycles/new">새 학기</Link>}>
        <section aria-labelledby="cycle-list-title"><div className="flex items-end justify-between border-b border-[var(--line)] pb-4"><h2 id="cycle-list-title" className="text-lg font-semibold">학기 목록</h2><span className="muted text-sm">총 {cycles.length}개</span></div>{cycles.length === 0 ? <div className="mt-6"><EmptyState title="아직 설정한 학기가 없습니다" description="첫 운영 학기를 정하면 프로그램을 만들 수 있습니다." action={<Link className="button-secondary" href="/admin/academic-cycles/new">첫 학기 설정</Link>} /></div> : <ol className="divide-y divide-[var(--line)]">{cycles.map((cycle) => <li key={cycle.id} className="grid gap-3 py-5 sm:grid-cols-[minmax(0,1fr)_10rem] sm:items-center"><span className="font-semibold">{cycle.academicYear}학년도 {cycle.term === "FIRST" ? "1" : "2"}학기</span><div className="sm:text-right"><StatusBadge>운영 기준</StatusBadge></div></li>)}</ol>}</section>
      </AdminWorkspace>
    </AppShell>
  );
}

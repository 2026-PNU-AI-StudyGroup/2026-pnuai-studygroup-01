...
export function UserStatusForm({ userId, name, isActive, activeResponsibilityCount }: { userId: string; name: string; isActive: boolean; activeResponsibilityCount: number }) {
  ...
  return (
    <form action={action} className="flex flex-wrap items-center gap-2 sm:justify-end">
      ...
      {isActive ? <ConfirmSubmitButton className="button-danger" confirmMessage={`${name} 계정을 비활성화하시겠습니까? 현재 로그인된 세션도 모두 종료됩니다.`} disabled={pending || activeResponsibilityCount > 0}><UiText>{pending ? "처리 중" : "비활성화"}</UiText></ConfirmSubmitButton> : <button className="button-secondary" disabled={pending}><UiText>{pending ? "처리 중" : "다시 활성화"}</UiText></button>}

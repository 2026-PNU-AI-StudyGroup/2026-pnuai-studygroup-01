  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath={`/admin/programs/${program.id}/settings`}>
      <AdminWorkspace
        ...
        actions={<><Link href={`/admin/programs/${program.id}/rubric`} className="button-secondary"><UiText>{"채점표 관리"}</UiText></Link><Link href={`/admin/programs/${program.id}/tracks`} className="button-secondary"><UiText>{"트랙 관리"}</UiText></Link><Link href={`/admin/programs/${program.id}/reports`} className="button-secondary"><UiText>{"제출물 요건"}</UiText></Link><Link href={`/announcements/new?target=program:${program.id}`} className="button-secondary"><UiText>{"공지 작성"}</UiText></Link><Link href="/admin/programs" className="button-secondary"><UiText>{"프로그램 목록"}</UiText></Link></>}
      >

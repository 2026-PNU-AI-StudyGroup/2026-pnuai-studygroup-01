...
function refresh(programId: string) { revalidatePath(`/admin/programs/${programId}/tracks`); revalidatePath(`/admin/programs/${programId}/settings`); revalidatePath("/topics"); revalidatePath("/projects/new"); revalidatePath("/professor/topics/new"); }
...
export async function deleteTrackAction(trackId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  try { const currentActor = await actor(); const confirmed = formData.get("confirmed") === "true"; const programId = String(formData.get("programId") ?? ""); const parsedImpact = confirmedImpactSchema.safeParse(Object.fromEntries(formData)); const outcome = await service().delete(currentActor, trackId, confirmed, parsedImpact.success ? { projectCount: parsedImpact.data.expectedProjectCount, voteCount: parsedImpact.data.expectedVoteCount, switchesVotingScope: parsedImpact.data.expectedSwitchesVotingScope } : undefined); if (outcome === "CONFIRMATION_REQUIRED") { const impact = await service().impact(currentActor, trackId); if (!impact) return { status: "error", message: "분과를 찾을 수 없습니다." }; return { status: "confirm", message: "연결 프로젝트는 미분과로 이동하고 이 프로그램의 표는 초기화됩니다.", ...impact }; } if (outcome === "NOT_FOUND") return { status: "error", message: "분과를 찾을 수 없습니다." }; refresh(programId); return { status: "success", message: "분과를 삭제했습니다." }; }

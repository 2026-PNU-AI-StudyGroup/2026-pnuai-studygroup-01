...
async function lockProgram(tx: PrismaTypes.TransactionClient, programId: string) {
  const rows = await tx.$queryRaw<Array<{
    ...
    SELECT "id", "lifecycleStatus", "startsAt", "endsAt"
...
function activeTeamWhere(programId: string, divisionId: string | null): PrismaTypes.TeamWhereInput {
  if (divisionId) return { programId, status: { not: "CLOSED" }, topic: { divisionId } };
  return {
    programId,
    status: { not: "CLOSED" },
    OR: [
      { topic: { divisionId: null } },
      { topic: { division: { rubricMode: "INHERIT_COMMON" } } },
    ],
  };
}
...
export async function createRubricAction(
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    if (program.lifecycleStatus !== "ACTIVE") return "CLOSED" as const;
    if (parsed.data.gradingDueAt <= now || parsed.data.gradingDueAt < program.startsAt || parsed.data.gradingDueAt > program.endsAt) return "DEADLINE" as const;
    if (divisionId) {
    ...
    const teams = await tx.team.findMany({ where: activeTeamWhere(programId, divisionId), select: { id: true } });
    if (teams.length) await tx.teamRubricEvaluation.createMany({ data: teams.map((team) => ({ teamId: team.id, rubricId: rubric.id })), skipDuplicates: true });
  ...
  if (outcome === "CLOSED") return failure("운영 종료된 프로그램의 채점표는 변경할 수 없습니다.");
  if (outcome === "DEADLINE") return failure("채점 마감은 프로그램 운영 기간 안의 미래 시각이어야 합니다.");
...
export async function updateRubricAction(
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    if (parsed.data.gradingDueAt < program.startsAt || parsed.data.gradingDueAt > program.endsAt || (parsed.data.gradingDueAt.getTime() !== current.gradingDueAt.getTime() && parsed.data.gradingDueAt <= now)) return "DEADLINE" as const;
...
export async function archiveRubricAction(rubricId: string, programId: string, _state: RubricActionState): Promise<RubricActionState> {
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    await tx.teamRubricEvaluation.deleteMany({ where: { rubricId } });
...
async function mutateRubricStructure<T>(programId: string, rubricId: string, operation: (tx: PrismaTypes.TransactionClient) => Promise<T>) {
  return prisma.$transaction(async (tx) => {
    const program = await lockProgram(tx, programId);
    if (!program || program.lifecycleStatus !== "ACTIVE") return null;
...
export async function setDivisionRubricModeAction(programId: string, divisionId: string, mode: "INHERIT_COMMON" | "CUSTOM", _state: RubricActionState): Promise<RubricActionState> {
  ...
  const outcome = await prisma.$transaction(async (tx) => {
    ...
    const teamIds = (await tx.team.findMany({ where: { programId, topic: { divisionId } }, select: { id: true } })).map((team) => team.id);
    if (teamIds.length && await tx.rubricScore.findFirst({ where: { evaluation: { teamId: { in: teamIds } } }, select: { id: true } })) return "SCORED" as const;
    ...
    if (mode === "CUSTOM") {
      ...
      if (teamIds.length && commonIds.length) await tx.teamRubricEvaluation.deleteMany({ where: { teamId: { in: teamIds }, rubricId: { in: commonIds } } });
      for (const source of common) {
        ...
        if (teamIds.length) await tx.teamRubricEvaluation.createMany({ data: teamIds.map((teamId) => ({ teamId, rubricId: copy.id })), skipDuplicates: true });
      }
    } else {
      const customIds = (await tx.rubricDefinition.findMany({ where: { programId, divisionId }, select: { id: true } })).map((rubric) => rubric.id);
      if (customIds.length) {
        await tx.teamRubricEvaluation.deleteMany({ where: { rubricId: { in: customIds } } });
      ...
      if (teamIds.length && commonIds.length) await tx.teamRubricEvaluation.createMany({ data: teamIds.flatMap((teamId) => commonIds.map((rubricId) => ({ teamId, rubricId }))), skipDuplicates: true });
    }
  ...
  if (outcome === "CLOSED") return failure("운영 종료된 프로그램의 분과 채점표는 변경할 수 없습니다.");

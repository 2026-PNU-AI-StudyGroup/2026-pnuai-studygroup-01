...
describe("ProgramVoteResults", () => {
  it("투표 예정 상태에도 정책과 후보 요약을 표시한다", () => {
    render(<ProgramVoteResults results={baseResults} refreshedAt="2026. 8. 19. 12:00:00" policySettingsHref="/admin/programs/program-1/settings#voting-policy" />);
    ...
    expect(screen.getByRole("link", { name: "투표 정책 수정" })).toHaveAttribute("href", "/admin/programs/program-1/settings#voting-policy");
  });
  ...
  it("분과별 결과에 섹션별 프로젝트 수와 총 표 수를 표시하고 기명 투표자는 접어 둔다", () => {
    ...
    render(<ProgramVoteResults results={results} refreshedAt="2026. 8. 20. 12:00:00" policySettingsHref="/admin/programs/program-1/settings#voting-policy" />);
  ...
  it("영어에서도 분과 결과 헤더를 division으로 표시한다", () => {
    ...
    render(<I18nProvider locale="en"><ProgramVoteResults results={results} refreshedAt="2026. 8. 20. 12:00:00" policySettingsHref="/admin/programs/program-1/settings#voting-policy" /></I18nProvider>);

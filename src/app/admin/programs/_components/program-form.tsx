...
export function ProgramForm({ successHref }: { successHref?: string }) {
  ...
  return <form action={action} aria-busy={pending} className="admin-panel overflow-hidden">
    <section className="grid gap-5 px-5 py-7 sm:grid-cols-2 sm:px-6" aria-labelledby="program-identity-title">
    ...
    <section className="grid gap-5 border-t border-[var(--line)] px-5 py-7 sm:grid-cols-2 sm:px-6" aria-labelledby="program-period-title">
    ...
    <fieldset className="grid gap-3 border-t border-[var(--line)] px-5 py-7 sm:px-6">
    ...
    <section className="border-t border-[var(--line)] px-5 py-7 sm:px-6" aria-labelledby="student-project-creation-title">
      <label className="flex cursor-pointer gap-3">
    ...
    <div className="flex justify-end border-t border-[var(--line)] px-5 py-5 sm:px-6"><button type="submit" className="button-primary max-sm:w-full" disabled={pending}><UiText>{pending ? "등록 중" : "초안 등록"}</UiText></button></div>{state.message ? <p role={state.status === "error" ? "alert" : "status"} aria-live="polite" className={`px-5 pb-5 sm:px-6 ${state.status === "error" ? "text-[var(--danger)]" : "text-[var(--success)]"}`}><UiText>{state.message}</UiText></p> : null}
  </form>;
}

...
export function ProgramForm({ cycles, successHref }: { cycles: AcademicCycleRecord[]; successHref?: string }) {
  ...
  return <form action={action} aria-busy={pending} className="grid gap-5 border-t-2 border-[var(--ink)] pt-6 sm:grid-cols-2">
    ...
    <div className="flex justify-end border-t border-[var(--line)] pt-6 sm:col-span-2"><button type="submit" className="button-primary" disabled={pending || !cycles.length}>{pending ? "등록 중" : "초안 등록"}</button></div>{state.message ? <p role={state.status === "error" ? "alert" : "status"} aria-live="polite" className={state.status === "error" ? "text-[var(--danger)] sm:col-span-2" : "text-[var(--success)] sm:col-span-2"}>{state.message}</p> : null}
  </form>;
}

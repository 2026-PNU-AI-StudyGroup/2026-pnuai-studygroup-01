...
export function ProgramStatusForm({ id, status }: { id: string; status: "DRAFT" | "OPEN" | "CLOSED" }) {
  ...
  return <form action={action} className="text-right"><input type="hidden" name="programId" value={id} /><input type="hidden" name="status" value={status === "DRAFT" ? "OPEN" : "CLOSED"} />{status === "DRAFT" ? <button className="button-primary" disabled={pending}><UiText>{pending ? "처리 중" : "공개"}</UiText></button> : <ConfirmSubmitButton className="button-danger" confirmMessage="프로그램을 마감하면 공개 주제와 팀원 모집도 함께 마감됩니다. 계속하시겠습니까?" disabled={pending}><UiText>{pending ? "처리 중" : "마감"}</UiText></ConfirmSubmitButton>}{state.message ? <p aria-live="polite" className={`mt-1 text-xs ${state.status === "error" ? "text-[var(--danger)]" : "text-[var(--success)]"}`}><UiText>{state.message}</UiText></p> : null}</form>;
}

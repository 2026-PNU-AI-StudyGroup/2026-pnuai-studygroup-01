...
export default async function ProfessorHistoryPage() {
  ...
  return (
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/admin/professors/history">
      <AdminWorkspace currentPath="/admin/professors/history" eyebrow="교수 권한 · 변경 이력" title="교수 권한 변경 이력" description="교수 권한을 허용하거나 회수한 기록과 처리자를 시간순으로 확인합니다." actions={<Link className="button-secondary" href="/admin/professors">권한 목록으로</Link>}>

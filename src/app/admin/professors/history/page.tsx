    ...
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/admin/professors/history">
      <AdminWorkspace currentPath="/admin/professors/history" eyebrow="교수 권한 · 변경 이력" title="교수 권한 변경 이력" description="권한을 허용하거나 회수한 기록과 처리자를 시간순으로 살펴봅니다." actions={<Link className="button-secondary" href="/admin/professors">권한 목록</Link>}>
        <section aria-labelledby="professor-audit-title">
          ...
          {entries.length === 0 ? <div className="mt-6"><EmptyState title="아직 권한 변경 기록이 없습니다" description="권한을 바꾸면 기록이 자동으로 남습니다." action={<Link className="button-secondary" href="/admin/professors/new">교수 이메일 추가</Link>} /></div> : <ol className="divide-y divide-[var(--line)]">{entries.map((entry) => <li key={entry.id} className="grid gap-2 py-5 text-sm sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"><div><strong>{entry.targetEmail}</strong><p className="muted mt-1 text-xs">처리자 {entry.actorName} · {entry.action === "PROFESSOR_ACCESS_GRANTED" ? "교수 권한 허용" : "교수 권한 회수"}</p></div><time className="muted text-xs" dateTime={entry.createdAt.toISOString()}>{koreanDateTime.format(entry.createdAt)}</time></li>)}</ol>}
        </section>
      </AdminWorkspace>
    </AppShell>
  );

...
export function ProfessorAccessForm() {
  ...
  return (
    <form action={action} className="grid gap-4 border-y border-[var(--line)] bg-[var(--surface-subtle)] px-4 py-6 sm:grid-cols-[minmax(0,1fr)_auto] sm:px-6">
      ...
      <button type="submit" disabled={pending} className="button-primary self-end">{pending ? "등록 중" : "교수 권한 허용"}</button>
      {state.message ? <p aria-live="polite" className={`sm:col-span-2 ${state.status === "error" ? "text-red-700" : "text-green-700"}`}>{state.message}</p> : null}
    </form>
  );
}

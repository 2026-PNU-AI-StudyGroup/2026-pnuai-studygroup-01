...
export function StudentProfileForm({ profile }: { profile: StudentProfile | null }) {
  ...
  return (
    <form action={action} aria-busy={pending} className="grid gap-5 rounded-[var(--radius-panel)] bg-[var(--surface-subtle)] p-5 sm:grid-cols-2 sm:p-6">
      ...
      <button type="submit" disabled={pending} className="button-primary justify-self-start max-sm:w-full">{pending ? "저장 중" : "프로필 저장"}</button>
      {state.message ? <p role={state.status === "error" ? "alert" : "status"} aria-live="polite" className={`self-center text-sm font-semibold ${state.status === "error" ? "text-[var(--danger)]" : "text-[var(--success)]"}`}>{state.message}</p> : null}
    </form>
  );
}

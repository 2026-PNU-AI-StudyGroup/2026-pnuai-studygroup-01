    ...
    <AppShell role={actor.role} userId={actor.id} userName={actor.name} currentPath="/notifications">
      <AccountSectionLayout role={actor.role} currentPath="/notifications"><div className="space-y-10">
        ...
        <section aria-labelledby="notification-list-title">
          ...
          {data.items.length === 0 ? <EmptyState title="새로운 알림이 없습니다" description="지원 결과와 마감 일정이 생기면 바로 알려드릴게요." /> : <ol className="divide-y divide-[var(--line)] border-y border-[var(--line)]">{data.items.map((notification) => <li key={notification.id} className={`${notification.readAt ? "py-6" : "border-l-2 border-l-[var(--accent)] py-6 pl-4 sm:pl-6"} record-row`}>

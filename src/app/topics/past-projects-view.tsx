      ...
      {projects.length === 0 ? <EmptyState title={hasFilters ? "조건에 맞는 프로젝트가 없습니다" : "아직 지난 프로젝트가 없습니다"} description={hasFilters ? "검색어, 학년도 또는 프로그램 분류를 바꿔 다시 찾아보세요." : "최종 보고서 승인 후 팀이 종료되면 선배 프로젝트와 결과물이 이곳에 쌓입니다."} action={hasFilters ? <Link className="button-secondary" href="/topics?view=past">전체 프로젝트 보기</Link> : undefined} /> : <>
        ...
        <ol className="divide-y divide-[var(--line)] border-b border-[var(--line)]">{projects.map((project) => {
          ...
          return <li key={project.id} className="py-7"><article aria-labelledby={`past-project-${project.id}`} className="grid grid-cols-[4.75rem_minmax(0,1fr)] gap-x-4 gap-y-4 lg:grid-cols-[5rem_minmax(16rem,1.5fr)_minmax(10rem,.8fr)_minmax(8rem,.6fr)_7rem] lg:gap-x-5">
            <div><p className="inline-flex min-h-11 min-w-16 items-center justify-center rounded-lg border border-[var(--accent)] text-lg font-black text-[var(--accent)] lg:min-h-0 lg:min-w-0 lg:justify-start lg:border-0 lg:text-base lg:text-[var(--ink)]">{project.academicYear}</p></div>
            ...
            <details className="group col-span-2 border-t border-[var(--line)] pt-3 lg:col-span-5"><summary className="snap-color flex min-h-11 cursor-pointer list-none items-center justify-between gap-4 text-sm font-extrabold text-[var(--accent)]">설명과 결과물 보기<span aria-hidden="true" className="group-open:rotate-180">↓</span></summary><div className="grid gap-6 pb-2 pt-3 lg:grid-cols-[minmax(0,1fr)_16rem]"><div><TranslatedText text={project.topicDescription} className="muted whitespace-pre-wrap leading-7" /><p className="muted mt-4 text-sm">참여자 {project.memberNames.join(", ")}</p></div><div><p className="text-sm font-extrabold">공개 결과물</p>{project.artifacts.length ? <ul className="mt-2 grid gap-1">{project.artifacts.map((artifact) => <li key={artifact.id}>{artifact.fileId ? <a className="button-quiet min-h-11 w-full justify-start px-0 text-left text-[var(--accent)]" href={`/api/files/${artifact.fileId}`}>{artifactType[artifact.type]} · {artifact.title}</a> : artifact.externalUrl ? <a className="button-quiet min-h-11 w-full justify-start px-0 text-left text-[var(--accent)]" href={artifact.externalUrl} target="_blank" rel="noreferrer">{artifactType[artifact.type]} · {artifact.title}<span className="sr-only"> 새 창</span></a> : <span className="muted flex min-h-11 items-center text-sm">{artifactType[artifact.type]} · {artifact.title}</span>}</li>)}</ul> : <p className="muted mt-2 text-sm">공개된 결과물이 없습니다.</p>}</div></div></details>
          </article></li>;
        })}</ol>

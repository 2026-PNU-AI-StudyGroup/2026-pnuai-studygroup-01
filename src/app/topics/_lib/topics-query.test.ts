...
describe("topicsHref", () => {
  it("진행 중 화면의 문맥과 필터를 안정된 순서로 직렬화한다", () => {
    expect(topicsHref({
      ...
      page: 2,
    })).toBe("/topics?programId=program-1&divisionId=division-1&q=%EA%B8%B8+%EC%B0%BE%EA%B8%B0&teamStatus=formed&reportStatus=overdue&page=2");
  });

  it("지난 화면 문맥은 유지하고 기본값은 URL에서 생략한다", () => {
    expect(topicsHref({ view: "past", programId: "program-1", teamStatus: "all", reportStatus: "all", page: 1 }))
...
describe("hasTopicsFilters", () => {
  it("검색·분과·팀 구성·보고서 조건만 실제 필터로 취급한다", () => {
    expect(hasTopicsFilters({})).toBe(false);
    expect(hasTopicsFilters({ teamStatus: "all", reportStatus: "all" })).toBe(false);
    expect(hasTopicsFilters({ q: "길찾기" })).toBe(true);
    expect(hasTopicsFilters({ divisionId: "UNASSIGNED" })).toBe(true);
    expect(hasTopicsFilters({ reportStatus: "submitted" })).toBe(true);
  });
});

...
/** 집계표에서 보낸 팀별 칸. 이름은 `award:<팀 id>`, `number:<팀 id>` 로 온다. */
const AWARD_FIELD_PREFIX = "award:";
const NUMBER_FIELD_PREFIX = "number:";

/**
 * 팀 번호와 오프라인 심사 결과를 집계표에서 한 번에 적는다.
 *
 * 둘 다 그 표를 보고 정하므로 입력칸도 같은 표에 둔다. 팀 이름을 손으로 옮겨 적을 일이
 * 없어 `5 Guys` 와 `5Guys` 처럼 띄어쓰기가 어긋나 엉뚱한 팀에 값이 들어가는 사고가 안 난다.
 * 빈 칸은 지운다는 뜻이라 그대로 보낸다.
 */
export async function saveProgramTeamResultsAction(_state: ProgramActionState, formData: FormData): Promise<ProgramActionState> {
  ...
  // 번호는 행사에서 부르는 값이라 0 이나 음수를 쓰지 않는다.
  const numberSchema = z.coerce.number().int().min(1).max(9999);
  const numbers = new Map<string, number | null>();
  for (const [key, value] of formData.entries()) {
    if (!key.startsWith(NUMBER_FIELD_PREFIX) || typeof value !== "string") continue;
    const teamId = key.slice(NUMBER_FIELD_PREFIX.length);
    if (value.trim() === "") { numbers.set(teamId, null); continue; }
    const number = numberSchema.safeParse(value);
    if (!number.success) return { status: "error", message: "팀 번호는 1부터 9999 사이의 숫자로 적어 주세요." };
    numbers.set(teamId, number.data);
  }
  const entries: Array<{ teamId: string; number: number | null; award: string | null }> = [];
  for (const [key, value] of formData.entries()) {
    ...
    const teamId = key.slice(AWARD_FIELD_PREFIX.length);
    entries.push({ teamId, number: numbers.get(teamId) ?? null, award: award.data || null });
  }
  if (entries.length === 0) return { status: "error", message: "저장할 팀이 없습니다." };
  try {
    await new ProgramTeamResultService(new PrismaProgramTeamResultRepository(prisma)).save(await actor(), programId.data, entries);
  } catch (error) {
    if (error instanceof InvalidProjectProgramError || error instanceof ProjectProgramOperationError) return { status: "error", message: error.message };
    throw error;
  }
  refreshManagement(programId.data, "results");
  return { status: "success", message: "팀 번호와 수상 내역을 저장했습니다." };
}

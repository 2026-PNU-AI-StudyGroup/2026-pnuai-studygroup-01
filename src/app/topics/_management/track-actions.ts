...
export type TrackActionState = { status: "idle" | "error" | "success" | "confirm"; message: string; projectCount?: number; voteCount?: number; rubricCount?: number; switchesVotingScope?: boolean };
...
const confirmedImpactSchema = z.object({
  ...
  expectedRubricCount: z.coerce.number().int().min(0),
...
export async function createTrackAction(programId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  ...
  catch (error) { unstable_rethrow(error); return { status: "error", message: error instanceof Error ? error.message : "분과를 추가할 수 없습니다." }; }
}
export async function renameTrackAction(trackId: string, programId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  ...
  catch (error) { unstable_rethrow(error); return { status: "error", message: error instanceof Error ? error.message : "분과 이름을 변경할 수 없습니다." }; }
}
export async function moveTrackAction(trackId: string, programId: string, direction: "up" | "down", _previous: TrackActionState): Promise<TrackActionState> {
  void _previous;
  try { const ok = await service().move(await actor(), trackId, direction); if (!ok) return { status: "error", message: "분과를 찾을 수 없습니다." }; refresh(programId); return { status: "success", message: "분과 이름을 변경했습니다." }; }
  catch (error) { unstable_rethrow(error); return { status: "error", message: error instanceof Error ? error.message : "순서를 변경할 수 없습니다." }; }
}
export async function deleteTrackAction(trackId: string, programId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  try { const currentActor = await actor(); const confirmed = formData.get("confirmed") === "true"; const parsedImpact = confirmedImpactSchema.safeParse(Object.fromEntries(formData)); const outcome = await service().delete(currentActor, trackId, confirmed, parsedImpact.success ? { projectCount: parsedImpact.data.expectedProjectCount, voteCount: parsedImpact.data.expectedVoteCount, rubricCount: parsedImpact.data.expectedRubricCount, switchesVotingScope: parsedImpact.data.expectedSwitchesVotingScope } : undefined); if (outcome === "SCORED_RUBRIC") return { status: "error", message: "이 분과 팀에 저장된 평가 점수가 있어 분과를 삭제할 수 없습니다." }; if (outcome === "PROGRAM_CLOSED") return { status: "error", message: "운영 종료된 프로그램의 분과는 삭제할 수 없습니다." }; if (outcome === "CONFIRMATION_REQUIRED") { const impact = await service().impact(currentActor, trackId); if (!impact) return { status: "error", message: "분과를 찾을 수 없습니다." }; return { status: "confirm", message: "연결 프로젝트는 미분과로 이동하고 이 프로그램의 표는 초기화됩니다.", ...impact }; } if (outcome === "NOT_FOUND") return { status: "error", message: "분과를 찾을 수 없습니다." }; refresh(programId); return { status: "success", message: "분과를 삭제했습니다." }; }
  catch (error) { unstable_rethrow(error); return { status: "error", message: error instanceof Error ? error.message : "분과를 삭제할 수 없습니다." }; }
}

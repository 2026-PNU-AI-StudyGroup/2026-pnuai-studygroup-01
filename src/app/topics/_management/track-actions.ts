...
function refresh() { revalidatePath("/topics"); revalidatePath("/professor/topics/new"); }
...
export async function createTrackAction(programId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  ...
  try { const outcome = await service().create(await actor(), programId, String(formData.get("name") ?? "")); if (outcome === "DUPLICATE") return { status: "error", message: "이미 있는 분과 이름입니다." }; if (outcome === "NOT_FOUND") return { status: "error", message: "프로그램을 찾을 수 없습니다." }; refresh(); return { status: "success", message: "분과를 추가했습니다." }; }
...
export async function renameTrackAction(trackId: string, programId: string, _previous: TrackActionState, formData: FormData): Promise<TrackActionState> {
  try { const outcome = await service().rename(await actor(), trackId, String(formData.get("name") ?? "")); if (outcome === "DUPLICATE") return { status: "error", message: "이미 있는 분과 이름입니다." }; if (outcome === "NOT_FOUND") return { status: "error", message: "분과를 찾을 수 없습니다." }; refresh(); return { status: "success", message: "분과 이름을 변경했습니다." }; }
...
export async function moveTrackAction(trackId: string, programId: string, direction: "up" | "down", _previous: TrackActionState): Promise<TrackActionState> {
  void _previous;
  try { const ok = await service().move(await actor(), trackId, direction); if (!ok) return { status: "error", message: "분과를 찾을 수 없습니다." }; refresh(); return { status: "success", message: "분과 이름을 변경했습니다." }; }

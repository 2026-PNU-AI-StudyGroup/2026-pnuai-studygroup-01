...
describe("ProjectExplorerLayout", () => {
  it("학생에게 진행 중과 지난 프로젝트 전환 및 실제 보조 경로를 제공한다", () => {
    ...
    expect(screen.getAllByRole("link", { name: "지난 프로젝트" })).toHaveLength(2);
  ...
  it("교수에게 지난 프로젝트 선택과 지도 프로젝트 경로를 제공한다", () => {
    render(<ProjectExplorerLayout role="PROFESSOR" view="past"><p>프로젝트 내용</p></ProjectExplorerLayout>);

    expect(screen.getAllByRole("link", { name: "지난 프로젝트" }).every((link) => link.getAttribute("aria-current") === "page")).toBe(true);

...
describe("ProjectSearchForm", () => {
  ...
  it("관리자 프로젝트 조건을 검색 후에도 보존한다", () => {
    const { container } = render(<ProjectSearchForm view="active" programId="program-1" query="" teamStatus="formed" reportStatus="overdue" />);

    expect(container.querySelector('input[name="teamStatus"]')).toHaveValue("formed");
    expect(container.querySelector('input[name="reportStatus"]')).toHaveValue("overdue");
  });
});

 ...
 * 검사가 다섯 곳(개인 지원·팀 지원·초대 보내기·초대 수락·지원 승인)에 흩어져 있었기
 * 때문에 한 곳만 고치면 다른 경로로 다시 들어온다. 조건을 여기 한 번만 적는다.
 *
 * 지원 승인(`prisma-topic-application-acceptance.ts`)은 나중에 발견됐다. 지원할 때는
 * 프로그램 단위로 막는데 승인할 때는 그 주제 팀만 봐서, 한 프로그램의 서로 다른 주제
 * 둘에 지원해 두면 둘 다 승인돼 팀 두 곳에 소속됐다.
 *

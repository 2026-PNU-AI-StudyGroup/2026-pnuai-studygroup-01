...
export class PrismaProgramReportDefinitionRepository implements ProgramReportDefinitionWriter {
  ...
  create(input: { programId: string; title: string; dueAt: Date; required: boolean; actorId: string; now: Date }) {
    return this.client.$transaction(async (tx): Promise<ProgramReportDefinitionOutcome> => {
      ...
      const definition = await tx.programReportDefinition.create({
        data: { programId: input.programId, title: input.title, dueAt: input.dueAt, required: input.required, position: (last?.position ?? -1) + 1 },
      });
      ...
      if (teams.length) {
        await tx.report.createMany({
          data: teams.map(({ id: projectTeamId }) => ({ projectTeamId, definitionId: definition.id, titleSnapshot: input.title, dueAt: input.dueAt, required: input.required, submissionEnabled: true })),
        });
      }
...
async function lockProgram(tx: Prisma.TransactionClient, programId: string) {
  const rows = await tx.$queryRaw<Array<{ id: string; executionStartsAt: Date; executionEndsAt: Date }>>(Prisma.sql`
    SELECT "id", "executionStartsAt", "executionEndsAt"

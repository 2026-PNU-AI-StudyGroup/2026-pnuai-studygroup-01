...
export class PrismaTopicApplicationDecisionRepository implements TopicApplicationDecisionRepository {
  ...
  private rejectOnce(
    ...
    return this.client.$transaction(async (transaction) => {
      ...
      const result = await transaction.topicApplication.updateMany({ where: { id: { in: targets.map(({ id: targetId }) => targetId) }, status: "PENDING" }, data: { status: "REJECTED", decidedAt, reviewComment } });

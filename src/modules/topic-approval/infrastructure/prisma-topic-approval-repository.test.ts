...
describe("학생 제안 프로젝트의 기존 팀 연결", () => {
  ...
  it("승인 시 기존 팀원을 모두 확정 참여시키고 실행 팀을 바로 확정한다", async () => {
    ...
    const queryRaw = vi.fn()
      ...
      .mockResolvedValueOnce([{ id: "student-team-1", leaderId: "student-1", name: "기존 팀" }]);
    const transaction = {
      $queryRaw: queryRaw,
      topic: {
        findFirst: vi.fn(async () => ({
    ...
    expect(createApplications).toHaveBeenCalledWith({
      data: expect.arrayContaining([
        expect.objectContaining({ studentId: "student-1", status: "ACCEPTED", participantRole: "LEADER" }),
        expect.objectContaining({ studentId: "student-2", status: "ACCEPTED", participantRole: "MEMBER" }),
      ]),
    });
    expect(createExecutionTeam).toHaveBeenCalledWith({
      data: expect.objectContaining({ status: "CONFIRMED", name: "기존 팀" }),
      select: { id: true },
    });
    expect(createMembers).toHaveBeenCalled();
    expect(updateTopic).toHaveBeenCalledWith({
      where: { id: "topic-1" },
      data: { status: "PUBLISHED", publishedAt: requestedAt },
    });
  });
});

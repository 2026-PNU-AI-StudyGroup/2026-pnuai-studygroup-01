...
import { RecruitmentOperationError, RecruitmentService, type RecruitmentRepository } from "@/modules/recruitment/application/manage-recruitment";

function repository(overrides: Partial<RecruitmentRepository> = {}): RecruitmentRepository {
  return {
...
describe("학생 팀원 모집", () => {
  it("공개 모집, 내 작성 글, 보낸 지원 이력을 서로 다른 조회로 읽는다", async () => {
    ...
    const service = new RecruitmentService(repo, decisions());
  ...
  it("관리자의 지원 결정을 관리자 권한으로 위임한다", async () => {
    const repo = repository({ findDecisionTarget: vi.fn(async () => "topic-application") });
    const decisionRepository = decisions();
    decisionRepository.accept.mockResolvedValue("ACCEPTED");

    await new RecruitmentService(repo, decisionRepository).decide({ id: "admin", role: "ADMIN" }, "application", "ACCEPT");

    expect(repo.findDecisionTarget).toHaveBeenCalledWith("application", { actorId: "admin", isAdmin: true });
  ...
  it("구성 단계 팀원이 구조화 모집 글을 등록한다", async () => {
    const repo = repository({ createPost: vi.fn(async () => true) });
    await new RecruitmentService(repo, decisions()).createPost({ id: "student", role: "STUDENT" }, {
      teamId: "team", title: " 개발자 모집 ", content: " 함께 개발합니다 ", requiredSkills: [" TypeScript "], roleNeeded: " 백엔드 ", availability: " 주말 ",
    });
    expect(repo.createPost).toHaveBeenCalledWith(expect.objectContaining({ title: "개발자 모집", requiredSkills: ["TypeScript"] }));
  });

  it("비학생의 모집 글 생성을 저장 전에 거부한다", async () => {
    const repo = repository();
    await expect(new RecruitmentService(repo, decisions()).createPost({ id: "professor", role: "PROFESSOR" }, {

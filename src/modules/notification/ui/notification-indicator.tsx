...
export async function NotificationIndicator({ userId, active }: { userId: string; active: boolean }) {
  ...
  return (
    <Link
      ...
      className={`snap-color relative grid size-11 shrink-0 place-items-center rounded-lg ${active ? "bg-[var(--primary-subtle)] text-[var(--primary)]" : "text-[var(--muted)] hover:bg-[var(--surface-subtle)] hover:text-[var(--ink)]"}`}
    >

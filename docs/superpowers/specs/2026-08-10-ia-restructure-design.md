...
- 프로그램별 승인 대기 건수는 프로그램 사이드바와 관리 화면 제목에 표시하고 `/topics/manage/<programId>?approvals=pending`으로 연결한다.

## 승인 대기

- 학생은 대시보드의 `검토 중` 목록에서 자신의 지원과 프로젝트 등록을 함께 확인한다.
- 교수는 대시보드의 `승인 대기` 목록에서, 관리자는 프로그램별 `approvals=pending` 목록에서 요청을 처리한다.
- 등록 내용은 별도 상세 페이지가 아니라 목록 행의 모달에서 확인한다. 모달에는 주제, 설명, 팀장·팀원과 승인/반려만 표시한다.

...
이 문서는 현재 유효 정책 문서와 현재 작업 트리의 코드·스키마·운영 문서를 대조해 확인한 차이만 기록한다. 테스트는 근거로 사용하지 않았고, 시드 데이터는 현재 모델과 충돌하는 새 기록을 생성하는 경우에만 함께 확인했다. 현재 남은 항목은 모두 정책 문서가 기준이며, 표의 **고칠 쪽**이 정책과 다른 동작을 만드는 구현 또는 시드다.

- 대조 기준일: 2026-08-14
- 코드 기준: 현재 작업 트리의 `prisma/schema.prisma`, `src/modules`, `src/app` Server Action·Route Handler, `scripts`, 운영 문서
- 이 문서는 정책을 새로 결정하지 않는다. 현재 남은 항목은 확정 정책에 맞게 고칠 구현 위치를 추적한다.
...
| ID | 정책 문서 | 고칠 쪽 | 영향 | 현재 차이 |
| --- | --- | --- | --- | --- |
| M-01 | 맞음 | 프로젝트 수정 요청 처리 | 높음 | 프로젝트 내용 수정으로 모집 여부를 바꿀 수 있다. |
| M-02 | 맞음 | 모집 마감 처리 | 높음 | 종료된 프로그램에서도 모집 마감 처리가 가능하다. |
| M-03 | 맞음 | 교수 권한 회수 조회 | 높음 | 종료된 프로젝트가 있어도 교수 권한 회수를 막는다. |
| M-04 | 맞음 | 학생 연락처 저장 | 높음 | 학생 전화번호가 `User.phoneNumber`와 별도 프로필 값으로 나뉘어 저장된다. |
| M-05 | 맞음 | 프로젝트 설명 입력칸 | 낮음 | 프로젝트 설명 입력칸은 10,000자를 허용하지만 정책·서버 한도는 8,000자다. |
| M-06 | 맞음 | 프로필 사진 조회 권한 | 중간 | 프로젝트에서 나간 구성원도 같은 팀의 프로필 사진을 계속 볼 수 있다. |
| M-07 | 맞음 | 보고서 정의 배정 | 중간 | 종료되었거나 구성 중인 팀에도 새 보고서 정의를 할당한다. |
| M-08 | 맞음 | 공지 작성 처리 | 높음 | 새 프로그램 구성원 전용·팀 전용 공지를 만들 수 있다. |
| M-09 | 맞음 | 데모 시드 | 중간 | 상태 없는 프로젝트 팀 모델과 달리 `TEAM_CLOSED` 감사 기록을 계속 만든다. |

...
describe("관리 화면의 중복·가짜 컨트롤", () => {
  ...
  it("투표 정책이 없는 프로그램은 설정으로만 진입시킨다", async () => {
  ...
  it("투표 정책이 있는 프로그램은 단계 배지와 투표 현황 진입점을 제공한다", async () => {
    listPrograms.mockResolvedValue([{
      ...program,
      status: "OPEN",
      votingPolicy: {
    ...
    expect(screen.getByRole("link", { name: "투표 현황" })).toHaveAttribute("href", "/admin/programs/program-1/votes");
  });
